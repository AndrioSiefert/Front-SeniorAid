        ...
        <div className='max-w-7xl mx-auto py-10 px-4'>
            ...
            <div className='flex flex-col sm:flex-row sm:space-x-6'>
                {UserExperience.map((item, index) => (
                    <div
                        key={index}
                        className='bg-gradient-to-br from-blue-50 to-blue-100 shadow-lg rounded-lg p-4 text-center hover:scale-105 transform transition-all duration-300'>
                        <Image
                            src={item.photo}
                            alt={item.name}
                            width={100}
                        ...
                        <h3 className='text-sm font-medium text-gray-600'>{item.category}</h3>
                        <p className='mt-2 text-gray-700 italic'>{item.comment}</p>

                        <div className='flex justify-center mt-4 text-yellow-400'>
                            {[...Array(5)].map((_, starIndex) => (
                                <FaStar key={starIndex} className='animate-pulse' />
                            ))}
                        </div>
                    </div>
                ))}
